...
# ページ設定
st.set_page_config(
    page_title="TikTok Lite Strategy Simulator v3.1",
    page_icon="📱",
    layout="wide",
...
st.title("📱 TikTok Lite 運用戦略シミュレーター v3.1 (完全版)")
...
with tab3:
    ...
    w_immediate = sum(st.session_state.invite_types_df["即時報酬"] * st.session_state.invite_types_df["運用比率(%)"] / 100)
    w_task = sum(st.session_state.invite_types_df["完走報酬"] * st.session_state.invite_types_df["運用比率(%)"] / 100)
    ...
    final_video = st.session_state.video_rewards_df[st.session_state.video_rewards_df["有効"]]["報酬額"].sum()

    st.divider()
    st.subheader("🎁 チェックイン追加報酬")
    c1, c2 = st.columns(2)
    with c1: p1 = st.slider("1350円の確率", 0, 100, st.session_state.checkin_rewards["tier1_prob"])
    with c2: p2 = st.slider("2700円の確率", 0, 100-p1, st.session_state.checkin_rewards["tier2_prob"])
...
with tab1:
    ...
    st.table(pd.DataFrame([{"項目": "1招待あたり期待収益", "金額": f"¥{int(per_invite_revenue):,}"}]))

with tab4:
    st.subheader("📈 スプレッドシート実績分析")
    if st.button("🔄 データを取得して分析"):
        with st.spinner("同期中..."):
...
st.sidebar.markdown("---")
st.sidebar.caption("TikTok Lite Strategy Simulator v3.1")
