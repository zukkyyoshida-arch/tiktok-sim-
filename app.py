...
# ページ設定
st.set_page_config(
    page_title="TikTok Lite Strategy Simulator v3.3",
    page_icon="📱",
    layout="wide",
...
st.title("📱 TikTok Lite 運用戦略シミュレーター v3.3")
...
if 'checkin_rewards_df' not in st.session_state:
    st.session_state.checkin_rewards_df = pd.DataFrame([
        {"報酬名": "ティア1", "報酬額": 1350, "出現確率(%)": 40},
        {"報酬名": "ティア2", "報酬額": 2700, "出現確率(%)": 40},
        {"報酬名": "ティア3", "報酬額": 6750, "出現確率(%)": 20}
    ])
...
with tab3:
    ...
    total_prob = c_check["出現確率(%)"].sum()
    if total_prob != 100: st.warning(f"出現確率の合計を100%にしてください（現在: {total_prob}%）")
...
st.sidebar.markdown("---")
st.sidebar.caption("TikTok Lite Strategy Simulator v3.3")
